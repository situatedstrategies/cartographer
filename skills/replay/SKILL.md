...
4. Offer to `open` the replay. Inside it, **Replay the build** tells the story one move at a time, chapter by chapter, across sessions; the time strip under the map jumps to any minute; the branch filter, the Plain/Native voice switch, the Story/Map view switch and zoom are in the toolbar. `cartographer serve --open` shows every project on one dashboard.
